...
have a function from `User` to `Day` (let's say it's their birthday), and a function from `Day` to `ChuckNorrisFact`, then
composition gives you a function from `User` to `ChuckNorrisFact`.
...
Such composiitons are useful becasue they allow you to break your code into smaller, highly-specialized functions and test them in isolation.
Smaller functions are usually easier to read and maintain as we will soon see.

### Implementing function composition in Javascript

Let's reuse the same example (for the purpose of this demonstration, the functions will be very simple) :

```javascript
const getBirthday = user => user.birthday
const getChuchNorrisFactOfTheDay = date => "Chuck Norris has counted to infinity. Twice." 
```

You could write a higher-order function to compose those two functions.

```javascript
const getChuckNorrisFactForUser = user => getChuchNorrisFactOfTheDay(getBirthday(user))
```

But this isn't ideal. You've created a very specific function, that you will need to rewrite every time you want to perform
another composition. Furthermore, the body of this function isn't looking gret. Imagine if we had to compose three or even four functions
together, we'd end up with four levels of nested parentheses, a nightmare!

What you could do instead is write a `compose` function that can compose any number of functions together. Remember our friend `reduce` ?
...
Here we are using `reduceRight`, a close cousin of `reduce` that operates from right to left, this allows us
to keep the mathematical composition order. Notice also the `...` in the argument list. This is used to create a *variadic function*.
A variadic function is a function that can take any number of arguments. In other terms, it is a function of variable arity. The caller of
the function will pass as many arguments as they need to be processes, and from the body of the function, those arguments will be received
in the form of an array.

The double fat arrow is the sign of a higher-order function : the result of calling `compose` with several functions will itself be a function!

```javascript
const getChuckNorrisFactForUser = compose(
  getChuchNorrisFactOfTheDay,
  getBirthday
)
```
 Much cleaner, right ? Now we have a way of composing any list of functions. But the right-to-left composition can be confusing for many people. What
 if you want left-to-right composition instead ? Just use `reduce` instead of `reduceRight`. Left-to-right composition is often referred to as *piping*.
...
## Currying is a special form of partial application
